...
cd "$PROJECT_DIR"
taskset -c 0,1 ./rodis-server > "$LOG_DIR/error.log" &
PID=$!
...
echo "RUNNING..."
redis-benchmark -t set,get,ping,incr,append,expire -p $PORT -c $NUM_CONNS -P 16 -n $NUM_REQS > "$LOG_DIR/bench.log" 2>&1
